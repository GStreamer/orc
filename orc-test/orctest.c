...
orc_test_gcc_compile (OrcProgram * p)
{
  ...
  n = snprintf (dump_dis_filename, sizeof (dump_dis_filename), "%s-dump.dis",
      base);
  ...
  n = snprintf (cmd, sizeof (cmd), "gcc -Wa,-mregnames -Wall -c %s -o %s",
  ...
  n = snprintf (cmd, sizeof (cmd),
      "objdump -dr %s | sed 's/^[ 0-9a-f]*:/XXX:/' >%s", obj_filename,
      dis_filename);
#else
  n = snprintf (cmd, sizeof (cmd), "objdump -dr %s >%s", obj_filename,
      dis_filename);
#endif
  ORC_ASSERT (n < sizeof (cmd));
  ret = system (cmd);
  ...
  n = snprintf (cmd, sizeof (cmd), "diff -u %s %s", dis_filename,
      dump_dis_filename);
  ORC_ASSERT (n < sizeof (cmd));
  ret = system (cmd);
  if (ret != 0) {
...
orc_test_gcc_compile_c64x (OrcProgram * p)
{
  ...
  sprintf (cmd, C64X_PREFIX "cl6x -mv=6400+ " "-c %s", source_filename);
  ret = system (cmd);
  if (ret != 0) {
...
float_compare (OrcArray * array1, OrcArray * array2, int i, int j)
{
  ...
  switch (array1->element_size) {
    case 4:
      if (isnan (*(float *) ptr1) && isnan (*(float *) ptr2))
        return TRUE;
      if (*(float *) ptr1 == *(float *) ptr2)
        return TRUE;
      if (fabs (*(float *) ptr1 - *(float *) ptr2) < MIN_NONDENORMAL)
        return TRUE;
      return FALSE;
    case 8:
      /* FIXME */
...
orc_test_compare_output_full (OrcProgram * program, int flags)
{
  ...
  for (k = ORC_VAR_D1; k < ORC_VAR_D1 + 4; k++) {
    if (program->vars[k].size > 0) {
      if (!orc_array_compare (dest_exec[k - ORC_VAR_D1],
              dest_emul[k - ORC_VAR_D1], flags)) {
        ...
        for (l = ORC_VAR_D1; l < ORC_VAR_D1 + 4; l++) {
          if (program->vars[l].size > 0) {
            if (flags & ORC_TEST_FLAGS_FLOAT) {
              ...
              if (!float_compare (dest_emul[l - ORC_VAR_D1],
                      dest_exec[l - ORC_VAR_D1], i, j) != 0) {
                line_bad = TRUE;
              }
            } else {
  ...
  for (i = 0; i < 4; i++) {
    if (dest_exec[i])
      orc_array_free (dest_exec[i]);
    if (dest_emul[i])
      orc_array_free (dest_emul[i]);
  }
  for (i = 0; i < 8; i++) {
    if (src[i])
...
orc_test_get_program_for_opcode (OrcStaticOpcode * opcode)
{
  ...
  if (opcode->flags & ORC_STATIC_OPCODE_SCALAR) {
    ...
    args[n_args++] = orc_program_add_source (p, opcode->src_size[0], "s1");
    args[n_args++] = orc_program_add_source (p, opcode->src_size[1], "s2");
  }
...
orc_test_get_program_for_opcode_const (OrcStaticOpcode * opcode)
{
  ...
  if (opcode->src_size[1] == 0) {
    args[n_args++] = orc_program_add_constant (p, opcode->src_size[0], 1, "c1");
  } else {
    args[n_args++] = orc_program_add_source (p, opcode->src_size[0], "s1");
