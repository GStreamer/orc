...
powerpc_emit_ld (OrcCompiler * compiler, int regd, int rega, int imm)
{
  ...
  ORC_ASM_CODE (compiler, "  ld %s, %d(%s)\n",
      powerpc_get_regname (regd), imm, powerpc_get_regname (rega));
  insn =
      (58 << 26) | (powerpc_regnum (regd) << 21) | (powerpc_regnum (rega) <<
      16);
  insn |= imm & 0xffff;
...
powerpc_emit_std (OrcCompiler * compiler, int regs, int rega, int offset)
{
  ...
  ORC_ASM_CODE (compiler, "  std %s, %d(%s)\n",
      powerpc_get_regname (regs), offset, powerpc_get_regname (rega));
  insn =
      (62 << 26) | (powerpc_regnum (regs) << 21) | (powerpc_regnum (rega) <<
      16);
  insn |= offset & 0xffff;
    ...
    orc_uint32 b, orc_uint32 c, orc_uint32 d)
{
  ...
  if (p->is_64bit) {
    ...
    powerpc_emit_ld (p, greg, greg, (int) ORC_STRUCT_OFFSET (OrcCode, exec));
  } else {
    powerpc_emit_lwz (p,
        greg,
        POWERPC_R3, (int) ORC_STRUCT_OFFSET (OrcExecutor, arrays[ORC_VAR_A2]));
